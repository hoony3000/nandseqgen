...
if __name__ == "__main__":
    ...
    with open("config.yaml", "r", encoding="utf-8") as file:
        ...
        num_dies = Dict["Architecture"]["num_dies"]
        num_planes = Dict["Architecture"]["num_planes"]
        num_blocks = Dict["Architecture"]["num_blocks"]
        pagesize = Dict["Architecture"]["pagesize"]
    ...
    tRST_ready = StateSeq.create(
        times=TimeSpec["tRST_ready"], states="reset busy ready"
    )
    ...
    tERSL_TLC = StateSeq.create(
        times=TimeSpec["tRST_pgm"], states="erase TLC suspend busy"
    )
    tPGMSL_TLC = StateSeq.create(
        times=TimeSpec["tRST_ready"], states="program TLC suspend busy"
    )

    # operations 정의 (수정중)
    # seq[0] : operation 의 execution time -> 추후 dictionary data 로 입력을 받을 예정
    name = "erase TLC"
    erase_TLC = Operation.create(
        name=name, seq=Nop(ExecTime[name]) + tWB + tERS_TLC + End(name), applyto="die"
    )
    name = "pgm LSB"
    pgm_LSB = Operation.create(
        name=name, seq=Nop(ExecTime[name]) + End(name), applyto="die"
    )
    name = "pgm CSB"
    pgm_CSB = Operation.create(
        name=name, seq=Nop(ExecTime[name]) + End(name), applyto="die"
    )
    name = "pgm MSB"
    pgm_MSB = Operation.create(
        name=name, seq=Nop(ExecTime[name]) + End(name), applyto="die"
    )
    name = "pgm MSB exec"
    pgm_MSB_exec = Operation.create(
        name=name, seq=Nop(ExecTime[name]) + End(name), applyto="die"
    )
    name = "read LSB"
    read_LSB = Operation.create(
        name=name,
        seq=Nop(ExecTime[name]) + tR_LSB + tRRC_TLC + End("read TLC"),
        applyto="die",
    )
    name = "read CSB"
    read_CSB = Operation.create(
        name=name,
        seq=Nop(ExecTime[name]) + tR_CSB + tRRC_TLC + End("read TLC"),
        applyto="die",
    )
    name = "read MSB"
    read_MSB = Operation.create(
        name=name,
        seq=Nop(ExecTime[name]) + tR_MSB + tRRC_TLC + End("read TLC"),
        applyto="die",
    )
    name = "dout 06h"
    dout_06h = Operation.create(
        name=name, seq=Nop(ExecTime[name]) + End(name), applyto="die"
    )
    name = "SR 70h"
    SR_70h = Operation.create(
        name=name, seq=Nop(ExecTime[name]) + End(name), applyto="die"
    )
    name = "SR 78h"
    SR_78h = Operation.create(
        name=name, seq=Nop(ExecTime[name]) + End(name), applyto="die"
    )
    name = "reset FAh"
    reset_FAh = Operation.create(
        name=name, seq=Nop(ExecTime[name]) + tRST_ready + End(name), applyto="die"
    )
    name = "erase TLC suspend"
    erase_TLC_suspend = Operation.create(
        name=name, seq=Nop(ExecTime[name]) + tERSL_TLC + End(name), applyto="die"
    )
    name = "erase TLC resume"
    erase_TLC_resume = Operation.create(
        name=name,
        seq=Nop(ExecTime[name]) + tERS_TLC.shift_time(-2 * MS) + End(name),
        applyto="die",
    )
    name = "program TLC suspend"
    program_TLC_suspend = Operation.create(
        name=name, seq=Nop(ExecTime[name]) + tERSL_TLC + End(name), applyto="die"
    )
    name = "program TLC resume"
    program_TLC_resume = Operation.create(
        name=name,
        seq=Nop(ExecTime[name]) + tPROGO.shift_time(-300 * US) + End(name),
        applyto="die",
    )
